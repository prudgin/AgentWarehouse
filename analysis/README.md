...
One subdirectory per investigation, dated `YYYY-MM-DD-<kebab-topic>/`. Each holds the scripts that produced the result and a canonical `REPORT.md`. Cross-cutting narrative across investigations lives in [analysis-landscape.md](analysis-landscape.md).
...
See the library template's analysis README for the full conventions, REPORT.md format, and workflow:
...
1. Start: `/start-analysis <topic>` (or manual: create the dir, copy the REPORT template, add a stub entry to [analysis-landscape.md](analysis-landscape.md)).
2. Investigate.
