# CLAUDE.md — 2021 Feed Trial Marine vs Non Marine vs LAP
...
This project is bidirectionally mirrored to `sharepoint_planning:PROJECTS/2021 Feed Trial Marine vs Non Marine vs LAP/`. The local directory and the SharePoint folder hold **the same shape**, with one exception: code does not push.
...
**WHAT**: Feed-formulation trial comparing diets formulated high in marine-based proteins/oils versus diets using land-animal proteins (LAP) versus blended formulations, in growing Murray cod. Conducted with Hayashikane, Biomar, and Ridley feeds. Scientific lead: Dr Trent D'Antignana (Nutrisea). Recordings: fish condition scoring at multiple timepoints (WP09, Whitton), histology at T0, physical feed properties on each candidate diet, and trial impact summarised in the in-trial sub-investigation `Data/Impact of feed diets 2021/`.

**WHY**: Marine-based protein is the historical default but is expensive and exposed to wild-catch fluctuations. The trial quantifies whether the cheaper LAP and blended alternatives can replicate growth performance and carcass quality, which is the basis for committing to a commercial diet formulation.

**HOW**:
- Approved proposal: `Proposal/MCA project proposal_Feed Trial 2021_22_(07.04.22)[42].docx`.
- Reports: `Reports/MCA_Final Feed Trial Report_Haya_Biomar & Ridley (20.03.23).pdf`, `Reports/MCA_Preliminary Feed Trial Report_2021 22 season (15.06.22)[33].pdf`.
- Condition scoring data: `Data/Condition scores whitton recording sheet.xlsx`, `Data/WP09 condition scores 23.5.22.xlsx`, `Data/Whitton condition scores 19.5.22.xlsx`.
- Histology + physical-properties contexts: `Data/Histology/`, `Articles/physical feed properties/`.
