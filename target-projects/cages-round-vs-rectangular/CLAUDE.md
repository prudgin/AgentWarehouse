# CLAUDE.md — 2021 Cages Round vs Rectangular
...
This project is bidirectionally mirrored to `sharepoint_planning:PROJECTS/2021 Cages Round vs Rectangular/`. The local directory and the SharePoint folder hold **the same shape**, with one exception: code does not push.
...
**WHAT**: Cage-design trial comparing round versus rectangular in-pond cage configurations for Murray cod (*Maccullochella peelii*) production. Run across two seasons (2022–2024) at the Bilbul site (Pond 3) with paired round/rectangular cages stocked under matched conditions. Scientific lead: Dr Trent D'Antignana (Nutrisea Pty Ltd) under contract to MCA. Recordings include weight checks, mortality, feed-per-day, water quality, harvests, and health checks across the two trial periods.

**WHY**: Cage geometry directly affects stocking density at depth, water-flow patterns inside the cage, and labour for husbandry tasks (feeding, sampling, harvest). The trial generates the data MCA needs to choose a default cage type for future expansion.

**HOW**:
- Approved proposal: `Proposal/Approved project proposal_Cage Type Trial (25.05.22).docx`.
- Trial data spans two campaigns under `Data/Cage design_DS data/` (the main trial) and `Data/231228 final weight check/` (the December-2023 follow-up sweep).
- Deliverables in `Reports/`: `MCA_Final Cage Design Report (05.04.24).pdf`, `MCA_Interim Cage Design Report_2021 22 (1.06.23).pdf`.
