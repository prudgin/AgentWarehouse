# CLAUDE.md — 2025 Chlorella and PSB Stage 1
...
This project is bidirectionally mirrored to `sharepoint_planning:PROJECTS/2025 Chlorella and PSB Stage 1/`. The local directory and the SharePoint folder hold **the same shape**, with one exception: code does not push.
...
**WHAT**: Step 1 of a combined trial establishing two foundational culture systems for downstream feed and pond work — (1) small-scale *Chlorella vulgaris* algal cultivation and (2) Bacillus-based probiotic (PSB) suspension preparation. Algae are cultured in 500 mL flasks scaled to 1 L bottles inside an enclosed lab area at Stanbridge with blackout curtains, LED grow lights, CO₂ supply, and a pressure cooker as autoclave. PSB batches (100 L) are prepared in repurposed carboys for weekly pond dosing (~66 L per 11.5 ML pond). Outcomes: contamination-free algal biomass; consistent, applicable PSB at pond scale.

**WHY**: Both systems are upstream prerequisites for Step 2 — larger-scale production and feeding/pond performance trials. Establishing low-cost, self-contained production is what unlocks downstream work without standing up a full microbiology lab.

**HOW**:
- Methods proposal: `Proposal/Proposal Step1_Combined_Trial_Chlorella_PSB.docx` and `Proposal/Dec 2025 proposal PSB.docx`.
- Reference reading on PSB: `Proposal/Photosynthetic bacteria (PSB) - purple non-sulphur bacteria (Rhodopseudomonas Palustris).pdf`.
- Budget and equipment list: `Proposal/Chlorella vulgaris cultivation_Small Scale_Budget List and Setup.pdf` and the `Expenses/` directory.
- Tracking data so far: `Data/RAS stocking 17-11-2025.xlsx`, `Data/Whitton_pond_biomass_oxygen_28-11-2025.xlsx`.
- This is Step 1; Step 2 (larger production / feed trial) will spin out as a follow-on project.
