# CLAUDE.md — 2026 Feeding Frequency Juvenile
...
This project is bidirectionally mirrored to `sharepoint_planning:PROJECTS/2026 Feeding Frequency Juvenile/`. The local directory and the SharePoint folder hold **the same shape**, with one exception: code does not push.
...
**WHAT**: Juvenile feeding-frequency trial (planned, in early setup). Designed as a follow-on to the 2026 Juvenile gut-evacuation trial: inter-meal spacing is chosen from clearance kinetics rather than guesswork. Cohorts mirror the gut-evac project — A (10–45 g, 3 mm pellet), B (45–80 g, 4.5 mm), C (80–150 g, 4.5/6.5 mm). Cage allocation is being driven by the Bilbul fish census (`Proposal/Bilbul_fish_census_2026-05-04.xlsx`).

**WHY**: Production wants empirical guidance on how often to feed juveniles of each size class. Once GER (gastric evacuation rate) curves are available from the sibling project, this trial validates the inter-meal spacing they suggest under cage conditions.

**HOW** — early state at migration:
- Cage census + planning materials live under `Proposal/` (`Bilbul_fish_census_2026-05-04.xlsx`, `labour plan.xlsx`).
- Feed-size reference: `Proposal/PGO - 039 Feed size chart.pdf`.
- No trial data yet — `Data/` and `Reports/` are empty placeholders.
- Sibling project for upstream kinetics: `~/ResearchProjects/2026 Juvenile gut evac/` and its `analysis/` outputs.
