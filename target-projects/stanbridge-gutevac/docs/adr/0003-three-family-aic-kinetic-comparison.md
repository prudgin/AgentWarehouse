...
- **Square-root** (`y = (√a − Kt)²`) assumes evacuation is proportional to bolus surface area — bolus erodes from the outside. Common in piscine GER literature (e.g. Talbot 2001).
...
At Stanbridge grow-out weights the bolus-surface-area reasoning may matter more than at Bilbul (larger boluses, more surface-area-driven mass transfer). That's a hypothesis the AIC table is positioned to surface, not a prior we should bake in.
...
- **Three fits per cohort instead of one.** Cheap.
- **Cohort-specific model choice.** Each cohort can have a different best-fit family. This is the correct outcome if cohort size genuinely changes evacuation kinetics. It complicates cross-cohort reporting — adopt the convention of reporting cohort-by-cohort and noting any cross-cohort kinetic pattern.
- **Cross-project comparability.** Both this project and the sibling Bilbul trial use the same three-family AIC framing, so headline numbers compare directly when both trials complete.
...
- Sibling `2026 Juvenile gut evac` project, `docs/adr/0003-three-family-aic-kinetic-comparison.md` (methodology source).
- Talbot 2001 (piscine feed-intake patterns) — surface-area reasoning. (To be staged in `Articles/` when the project is scaffolded.)
