# CLAUDE.md — 2023 Continuous Copper
...
This project is bidirectionally mirrored to `sharepoint_planning:PROJECTS/2023 Continuous Copper/`. The local directory and the SharePoint folder hold **the same shape**, with one exception: code does not push.
...
**WHAT**: Investigation into whether continuous low-dose copper treatment in pond water reduces *Chilodonella* (chylo) parasite incidence in Murray cod. Originated as a retrospective analysis of cages used in the round-vs-rectangular Nutrisea trial — many of those cages received copper, so the data is reusable for a copper-vs-no-copper comparison despite lacking dedicated replicates.

**WHY**: Chylodonella outbreaks are recurrent and expensive (per-treatment cost + lost growth). If continuous copper is preventive, it changes pond husbandry. The project surfaces the existing data to estimate the effect size and inform whether a dedicated controlled trial is warranted.

**HOW**:
- Project framing (meeting minutes): `Proposal/230427_Copper meeting.docx`.
- Communication: `Articles/Copper treatment project.msg`.
- Treatment dataset: `Data/Copper_DS data/`, plus `Data/Continuous copper treatments_v2.xlsx`.
- Final report: `Reports/MCA_Final Copper Treatment Report_2023 (26.03.24).pdf`.
- Costing: `Expenses/Copper expense sheet.xlsx`.
- Caveat per the meeting note: replicates are confounded with the round-v-rectangular project; a disclaimer should accompany any quantitative claim.
