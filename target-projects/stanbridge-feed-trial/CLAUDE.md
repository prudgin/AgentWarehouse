# CLAUDE.md — 2025 Stanbridge Feed Trial
...
This project is bidirectionally mirrored to `sharepoint_planning:PROJECTS/2025 Stanbridge Feed Trial/`. The local directory and the SharePoint folder hold **the same shape**, with one exception: code does not push.
...
**WHAT**: Stanbridge-site feed-ration refinement trial in freshwater static ponds — driving the ration delivered to Murray cod toward a more productive setpoint. Authored by Dr Erin Bubner and Dr Trent D'Antignana. Interim report drafted December 2025 (covering trial period through July 2025); a follow-up "bi reports" stream tracks bi-monthly updates.

**WHY**: Feed ration (kg feed per day per pond) is set conservatively by default — under-feeding leaves growth on the table; over-feeding wastes feed and accumulates pond-bottom load. The trial finds the productive setpoint specific to Stanbridge conditions.

**HOW**:
- The proposal lives inside the interim report itself, drafted by Bubner & D'Antignana.
- Interim report: `Reports/MCA_Interim Feed Ration Report_July 2025_v19.12.25.pdf` (Dec 2025 finalised version).
- Bi-monthly reports: `Reports/bi reports/`.
- Recorded data: `Data/` (11 files of trial data).
- Articles & site context: `Articles/stanbridge feed trial.pptx`, `Articles/stanbridge site map.pdf`.
