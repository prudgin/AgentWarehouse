# CLAUDE.md — 2025 RAS Feed Biomar
...
This project is bidirectionally mirrored to `sharepoint_planning:PROJECTS/2025 RAS Feed Biomar/`. The local directory and the SharePoint folder hold **the same shape**, with one exception: code does not push.
...
**WHAT**: Second-generation RAS feed trial with Biomar (succeeds the 2023 RAS Feed Biomar project). Trial 2.0 protocol with three stages tracked via timepoints T0–T3 in 2025 (T0 weights 25 Mar 2025; T1 weights 29–30 Jul 2025; T2 sampling 22–24 Oct 2025 plus a small-tanks cull 5 Nov 2025; T3 final harvest Feb 2026). PI ownership: Rod Remke (project owner, Bilbul RAS site manager); Lizzie Moore (technical owner). Includes finisher-feed stage 2 / stage 3 work in late 2025.

**WHY**: Trial 2.0 refines the 2023 protein-energy modelling using a longer time series and a finisher-feed stage — the goal is a single Biomar recipe MCA commits to as the RAS production diet.

**HOW**:
- Protocol: `Proposal/RAS feed trial 2.0 protocol.docx`.
- Stocking plan: `Data/RAS trial 2.0 proposed fish numbers.xlsx`.
- Tank allocation: `Reports/2025 Ras trial tank Allocation.pptx`.
- Per-timepoint data folders: `Data/T0 weights-sampling 25.3.25/`, `Data/T1 weight sampling 29-30.7.25/`, `Data/T2 sampling 22-24.10.25/`, `Data/T2 sampling 05.11.25 small tanks cull/`, `Data/T3 final harvest/`.
- Master copy data: `Data/master copy data/`, `Data/COMPLETED DATA SHEETS/`.
- Finisher feed sub-investigation: `Data/RAS trial 3 - finisher feeds/`.
- Templates: `Proposal/templates/`, `Data/RAS Trial Average weight check template.xlsx`.
- Trial agreement: `Proposal/trial agreement/`.
- Daily growth tracking: `Data/daily growth 18-11-2025.xlsx`.
- Trial-design discussion summary: `Proposal/Trial design next hatchery trial Discussion Summary UPDATED.pdf`.
- Reports: `Reports/Finisher Feed (stage 2) presentation 31-10-2025.pdf`, `Reports/Fish processing and analysis_BiomarMCA Trial 2025.pdf`, `Reports/feed name to colour conversion.pptx`.
- Pellet weight soak test: `Data/Pellet weight soak test 2.4.25 - 3.4.25.xlsx`.
