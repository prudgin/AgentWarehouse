...
This project is bidirectionally mirrored to `sharepoint_planning:PROJECTS/2022 Larval Weaning/`. The local directory and the SharePoint folder hold **the same shape**, with one exception: code does not push.
...
**WHAT**: Larval weaning trial — moving Murray cod larvae from live feed (Artemia, rotifers) onto dry weaning diet under controlled tank conditions. Includes the larval rearing setup itself (small system, tank diagrams), a defined weaning protocol, electricity-consumption costing, and post-trial statistical analysis.

**WHY**: Weaning is the critical bottleneck in MCA's hatchery flow — survival drops sharply if the transition is rushed or the live-feed phase is too brief. Quantifying the cost (electricity) and the survival/growth outcomes lets the hatchery decide weaning timing for production runs.

**HOW**:
- Weaning protocol + process: `Proposal/Larval weaning protocol.docx`, `Proposal/Larvae Weaning Process.docx`.
- Tank setup: `Proposal/Tank setup diagram.docx`.
- Final report: `Proposal/Larvae Weaning Report.docx` (filename has no "report" keyword but it's the consolidated write-up).
- Recorded data: `Data/Larvae weaning statistics .xlsx`, `Data/data analysis larvae weaning.xlsx`.
- Cost analyses: `Expenses/Electricity Consumption and Cost Estimation Larval Rearing Trial.docx`, `Expenses/WEANING TRIAL BUDGET.xlsx`, `Expenses/larval weaning invoices.pdf`.
- EndNote library: `Articles/Larval Weaning.enl`.
