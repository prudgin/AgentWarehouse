# CLAUDE.md — 2023 RAS Feed Biomar
...
This project is bidirectionally mirrored to `sharepoint_planning:PROJECTS/2023 RAS Feed Biomar/`. The local directory and the SharePoint folder hold **the same shape**, with one exception: code does not push.
...
**WHAT**: RAS (Recirculating Aquaculture System) feed trial — testing Biomar protein/energy formulation modelling in MCA's RAS units (T2–T29 tanks at Bilbul). Joint Aquna ↔ Biomar protocol (PI: Felipe Muller, R&D tech: Deepika Satchithananthan). Tracks growth, feed-per-day, water-temp, T0 initial weights, and Health-Check assessments at T0 and T1.

**WHY**: Biomar's proprietary formulation modelling claims to optimise the protein-energy tradeoff for the target species. Validating that under MCA's RAS conditions decides whether the formulation gets adopted broadly. Final-stage RAS feed becomes the baseline that the 2025 RAS Feed Biomar trial 2.0 then refines.

**HOW**:
- Protocols: `Proposal/230501_V2_Internal protocol AQUNA and Biomar_DS.docx`, `Proposal/230501_V5_Trial protocol AQUNA and Biomar_DS.docx`.
- Trial framing meeting: `Proposal/200723_RAS move out meeting.docx`.
- Main trial data: `Data/Biomar_DS data/`, `Data/Biomar Feed trial_Nursery data/`, `Data/Final Data Oct-2023/`.
- Health checks: `Data/Health Check T0/`, `Data/Health Check T1/`.
- Histology results: `Data/BSL histo results/`.
- Per-day feed + temp: `Data/BM trial feed per day + water temp.xlsx`.
- Growth tracking: `Data/Growth & Feed.xlsx`, `Data/Growth & Feed V1.xlsx`.
- Weight checks (3 dates × 2 versions): `Data/RAS Trial Average weight check 23-05-16*.xlsx`, `Data/RAS Trial Average weight check 23-08-09 FV.xlsx`.
- Carcass analysis (downstream): `Data/carcass analysis fish RAS trial.xlsx`.
- Initial-weight reports: `Reports/T2 - T22 - initial weights.pdf`, `Reports/T23 - T29 - initial weights.pdf`.
- Biomar slides + data sent: `Data/biomar slides/`, `Data/data sent to biomar FM/`.
- Downstream analysis at Deakin: `Data/deakin analysis of RAS fish/`.
- Budgeting: `Expenses/Biomar Budget.xlsx`, `Expenses/Biomar expenses.xlsx`, `Expenses/projected budget.xlsx`.
