...
- [0024-research-template-bidirectional-sharepoint-mirror.md](0024-research-template-bidirectional-sharepoint-mirror.md) — new `research/` template for "official" MCA projects, bidirectionally mirrored to `sharepoint_planning:PROJECTS/` via `rclone copy --update`; symmetric except for code (agent infrastructure mirrors too); deletes never propagate.
- [0025-power-platform-bundle-lives-with-its-consumer.md](0025-power-platform-bundle-lives-with-its-consumer.md) — the single-consumer Power Platform bundle (10 skills + 3 reference docs) is relocated out of the warehouse into `~/MicrosoftFlowsApps`; the warehouse keeps only the generic `tool-integration` skeleton and does not carry single-consumer integration bundles.
- [0026-pretooluse-hook-blocks-shell-sourcing-env.md](0026-pretooluse-hook-blocks-shell-sourcing-env.md) — templates (and the warehouse) ship a `PreToolUse` Bash hook that blocks shell-sourcing a `.env` (secret values with `&`/spaces leak into the session log when sourced); content-aware layer above ADR-0023's prefix-glob deny list. Safe path is `python-dotenv`.
