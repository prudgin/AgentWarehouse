...
- `.claude/skills/` at migration held 23 symlinks to warehouse canonical sources — 10 Power Platform (`power-platform-auth`, `pac-cli-linux`, `flows-*`, `apps-*`, `proxy-flow-scaffolding`, `anthropic-api-integration`) + 13 warehouse-general (`grill`, `to-prd`, `to-issues`, `triage`, `work-issue`, `finish`, `start-analysis`, `finish-analysis`, `diagnose`, `improve-codebase-architecture`, `zoom-out`, `file-cross-repo-ticket`, `check-inbox`).
...
**Update 2026-06-16 — Power Platform bundle relocated (this supersedes the "23 symlinks" and "`docs/reference/` is sparse" notes above).** The drift surprise was resolved structurally: with this confirmed as the only Power Platform project and none in prospect, the 10-skill Power Platform bundle + its 3 reference docs (`powerapps-gotchas.md`, `azure-cli-sharepoint-auth.md`, `pac-canvas-deprecation.md`) were moved out of the warehouse and made **first-party** in this repo. The PP skills under `.claude/skills/` are now real local directories (not symlinks); `docs/reference/` now holds the three docs. Only the **13 general** skills remain warehouse symlinks. The warehouse keeps just the generic `tool-integration` skeleton. See warehouse [ADR-0025](../adr/0025-power-platform-bundle-lives-with-its-consumer.md) and the project's own `docs/adr/0005-power-platform-bundle-is-first-party.md`. Net effect: no warehouse↔project drift surface for Power Platform knowledge, and `/migrate-project`/`/finish` no longer need a harvest-back step for it.
