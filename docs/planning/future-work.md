...
## From session (2026-06-27)

### Surface Perplexity web search/research as a warehouse capability

**What:** decide how the warehouse should expose Perplexity — now installed as a machine-wide `perplexity` MCP server giving `perplexity_ask` / `perplexity_search` / `perplexity_research` / `perplexity_reason` — as a first-class, warehouse-managed capability: a skill (e.g. a `/research` wrapper that drives the MCP tools in the project's vocabulary and citation conventions), a hook that nudges use when a task is research-shaped, and/or template-shipped MCP config so cold-started projects inherit it.
**Type:** proposal
**Why:** the server was wired up machine-wide this session (`mcpServers.perplexity` in `~/.claude.json`, key exported in `~/.bashrc` from the warehouse `.env`), but nothing in the warehouse references, documents, or standardises it — so the capability is invisible to the conventions and won't propagate to new projects. A warehouse skill/hook turns "when and how agents reach for live web research" into a versioned, reviewable decision instead of an ad-hoc per-session one.
**Open questions:** skill vs hook vs template-shipped MCP config — or a combination? given the tools are globally available the moment the MCP server is registered, does a warehouse skill add value beyond *documenting when to use them*? how does this relate to the global `deep-research` skill (present on this machine but absent from the warehouse inventory — itself an instance of the unmanaged machine-wide surface below)? do research outputs/citations land anywhere structured (`analysis/`, a REPORT) or stay inline?
**Links:** `skills/README.md`, [Warehouse should manage the machine-wide agent surface](#warehouse-should-manage-the-machine-wide-agent-surface), `~/.claude.json` (`mcpServers.perplexity`).

### Warehouse should manage the machine-wide agent surface

**What:** extend the warehouse's remit from per-project agent setup to the **machine-wide** surface — global skills in `~/.claude/skills/`, hooks and settings in `~/.claude/settings.json`, and MCP servers in `~/.claude.json` (`mcpServers`). Today these are hand-configured per machine, unversioned, and unreproducible. The warehouse should own a source of truth for them plus a way to install / sync / reconcile it onto a machine (and across machines via git), the same way it already owns per-project `.claude/skills/` symlinks into `skills/`.
**Type:** proposal
**Why:** machine-wide config is a blind spot. Concrete evidence this session: the `perplexity` MCP server and its `~/.bashrc` key export were set up entirely by hand, leaving nothing versioned or repeatable; and the global `deep-research` skill lives on this machine but is absent from the warehouse inventory. Each new machine starts from zero. Bringing the global surface under warehouse management gives it the same guarantees — versioned, reviewable, portable — that the warehouse already provides at the project level.
**Open questions:** what's the source of truth and install mechanism — a `global/` (or `machine/`) dir in the warehouse mirrored to `~/.claude/` via a sync skill, symlinks, or a generated installer? how are machine-specific bits (secrets like `PERPLEXITY_API_KEY`, per-machine paths) kept out of git while still managed? how does it reconcile when two machines diverge? does this subsume the narrower [Skill location for global skills](#skill-location-for-global-skills) open question, and compose with the warehouse-pinned [Global skill: "add idea to warehouse"](#global-skill-add-idea-to-warehouse-the-behaviour-change-channel) channel? should MCP servers be managed like skills, or separately (they carry secrets and per-machine availability)?
**Links:** [Skill location for global skills](#skill-location-for-global-skills), [Global skill: "add idea to warehouse" (the behaviour-change channel)](#global-skill-add-idea-to-warehouse-the-behaviour-change-channel), `skills/sudo-script/` (current ad-hoc global skill), `skills/README.md`.
